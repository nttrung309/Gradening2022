...
import com.example.gardeningappmanager.network.order.OrderRepository
import kotlinx.coroutines.Dispatchers

class OrderViewModel: ViewModel() {
    private  var orderResponse: OrderRepository = OrderRepository()
    ...
    fun getOrderByState(state: String) = liveData(Dispatchers.IO) {
        emit(Resource.loading(data = null))
        try {
            emit(Resource.success(data = orderResponse.getOrderByState(state)))
        } catch (exception: Exception) {
            emit(Resource.error(data = null, message = exception.message ?: "Error Occurred!"))
        }
    }
    fun postChangeOrderState(idOrder: Int, state: String) = liveData(Dispatchers.IO) {
        emit(Resource.loading(data = null))
        try {
            emit(Resource.success(data = orderResponse.postChangeOrderState(idOrder, state)))
        } catch (exception: Exception) {
            emit(Resource.error(data = null, message = exception.message ?: "Error Occurred!"))
        }
    }
